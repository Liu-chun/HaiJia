...
export default {
  ...
  methods: {
    ...
    updataTime() {
      this.startTime =
        this.timeList !== null ? this.timeList[0].replace(/-/g, "") : null;
      this.endTime =
        this.timeList !== null ? this.timeList[1].replace(/-/g, "") : null;
    ...
    plan() {
      ...
      this.selection.map((item) => {
        ...
        // 总经数
        this.allNum += item.TOTALBEAMAMOUNT1;
        // 经轴类型
        this.weavingType = item.BEAMTYPEID1;
        // 经轴名称
        this.weavingName = item.BEAMTYPENAME1;
        // 门幅
    ...
    handleCurrentChange(val) {
      ...
      this.getdata(val);
      this.pageIndex = val;
      // let searchVX = this.$store.state.advancedConditionObj;
      // searchVX.pageIndex = val;
      //   api.ProductionZZGetQuality2List(searchVX).then((res) => {
      //     if (res.data.isSuccess) {
      //       this.tableData = res.data.data.dataList;
      //       this.dataCount = res.data.data.dataCount; //总条数
      //       this.pageIndex = res.data.data.pageIndex; //当前第几页
      //       this.pageCount = res.data.data.pageCount; //总页数
      //       this.pageSize = res.data.data.pageSize; //每页显示条目个数
      //     } else {
      //       this.$message({
      //         showClose: true,
      //         message: res.data.message,
